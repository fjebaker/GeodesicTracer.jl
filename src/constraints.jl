# inplace
function constrain_all(m::AbstractMetricParams{T}, us::AbstractArray{T, 2}, vs::AbstractArray{T, 2}, μ) where {T<:Number}
    us_cols = eachcol(us)
    vs_cols = eachcol(vs)
...
function constrain_all(m::AbstractMetricParams{T}, us::SMatrix{4,M,T,L}, vs::SMatrix{4,M,T,L}, μ) where {M,T<:Number,L}
